...
interface SystemStats {
  ...
  systemMetrics: Array<{ metric: string; value: string; status: string; trend: number }>;
  topExams: Array<{ exam: string; attempts: number; avgScore: number; completion: number }>;
  recentErrors: Array<{ time: string; type: string; message: string; count: number }>;
}
  ...
  return (
    <Box>
      ...
      <Grid container spacing={3} mb={4}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" fontWeight={600} mb={2}>System Metrics</Typography>
            {loading ? (
              <Skeleton variant="rounded" height={250} />
            ) : (
              <TableContainer>
                <Table size="small">
                  ...
                  <TableBody>
                    {stats.systemMetrics.length > 0 ? stats.systemMetrics.map((metric) => (
                      <TableRow key={metric.metric}>
                        <TableCell>{metric.metric}</TableCell>
                        <TableCell align="center">{metric.value}</TableCell>
                        <TableCell align="center">
                          <Chip label={metric.status} size="small" color={metric.status === 'good' ? 'success' : 'warning'} />
                        </TableCell>
                        <TableCell align="center">
                          <Box display="flex" alignItems="center" justifyContent="center" gap={0.5}>
                            <TrendingUpIcon fontSize="small" sx={{ color: metric.trend >= 0 ? '#10b981' : '#ef4444', transform: metric.trend < 0 ? 'rotate(180deg)' : 'none' }} />
                      ...
                      <TableRow>
                        <TableCell colSpan={4} align="center">No metrics available</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" fontWeight={600} mb={2}>Recent Errors</Typography>
            {loading ? (
              <Skeleton variant="rounded" height={200} />
                ...
                {stats.recentErrors.map((error, index) => (
                  <ListItem key={index} sx={{ px: 0 }}>
                    ...
                    <ListItemText
                      ...
                      secondary={`${error.type} • ${error.time}`}
                      primaryTypographyProps={{ fontSize: '0.9rem' }}
                    />
                    <Chip label={`${error.count}x`} size="small" color="error" variant="outlined" />
                  </ListItem>
                ))}
              </List>
            ) : (
              <Box textAlign="center" py={4}>
                <Typography color="success.main" fontWeight={500}>✓ No recent errors</Typography>
                <Typography variant="caption" color="text.secondary">System running smoothly</Typography>
              </Box>
            )}
          </Paper>
        </Grid>
